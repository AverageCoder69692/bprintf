...
cursor_col = 0; 
  ...
  if (cursor_row >= VGA_HEIGHT);
      for (int r = 1; r < VGA_HEIGHT, r++) {  // r = row
          for (int c = 1; c < VGA_WIDTH< c++) {  //c = col
  ...
  for (unsigned int i = 0; fmt[i]; i++) {
    if (fmt[i] == '%' && fmt[i+1]) {
      ...
      switch (fmt[i]) {
        ...
        case 'p': {
            void* ptr p = va_arg(args, void*);
            print_hex((uintptr)ptr, true);
            break;
        }
        case '%': {
            putchar('%');
